...
function getNextFriday(date) {
  ...
  const diff = 5 - day > 0 ? 5 - day : 7 + (5 - day);
  copy.setDate(date.getDate() + diff);
  return copy;
}
...
function formatDate(date) {
  ...
  const hours =
    newDate.getUTCHours() <= 12
      ? newDate.getUTCHours()
      : newDate.getUTCHours() - 12;
  const time = newDate.getUTCHours() < 12 ? 'AM' : 'PM';
  const minutes = `0${newDate.getMinutes()}`.slice(-2);
  const seconds = `0${newDate.getSeconds()}`.slice(-2);
  return `${newDate.getMonth() + 1}/${newDate.getUTCDate()}/${newDate.getFullYear()}, ${hours}:${minutes}:${seconds} ${time}`;
}
...
function getCountWeekendsInMonth(month, year) {
  const duration = 32 - new Date(year, month - 1, 32).getDate();
  const firstDay = new Date(year, month - 1, 1).getDay();
  const lastDay = new Date(year, month - 1, duration).getDay();
  let result = 8;
  if (lastDay === 6) result += 1;
  if (lastDay === 0) result += 2;
  if (firstDay === 0) result += 1;
  if (firstDay === 6) result += 2;
  return result;
}
...
function getWeekNumberByDate(date) {
  ...
  if (day < 1) firstDay = new Date(date.getFullYear(), 0, 1 + 1);
  if (day > 1) firstDay = new Date(date.getFullYear(), 0, 1 + (7 - day) + 1);
  const diff = (date - firstDay) / 1000 / 3600 / 24 / 7;
  if (day === 1) return Math.ceil(diff);
  return Number.isInteger(diff) ? diff + 2 : Math.ceil(diff) + 1;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (date <= endDate) {
    let i = countWorkDays;
    while (i > 0 && date <= endDate) {
      date.setDate(firstDay);
      const day = `0${date.getDate()}`.slice(-2);
      const month = `0${date.getMonth() + 1}`.slice(-2);
      schedule.push(`${day}-${month}-${date.getFullYear()}`);
      firstDay += 1;
      i -= 1;
    }
    firstDay += countOffDays;
    date.setDate(firstDay);
  }
  return schedule;
}
